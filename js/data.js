...
const DATA = (function() {
  ...
  function loadNebimFile(file) {
    ...
    readExcelFile(file, (err, wb) => {
      ...
      const colMap = {
        ...
        altGrupAciklama: findColumn(keys, ['ALT', 'GRUP', 'AÇIKLAMA']) || findColumn(keys, ['ALTGRUPAÇIKLAMASI']) || findColumn(keys, ['ALT', 'GRUP']) || findColumn(keys, ['ALTGRUP']),
        ...
        takimDurumu: findColumn(keys, ['TAKIM', 'DURUMU']) || findColumn(keys, ['TAKIMDURUMU']),
      ...
      state.rawData = raw.map(r => ({
        ...
        takimDurumu: String(r[colMap.takimDurumu] || ''),
        takimKod: String(r[colMap.takimKod] || ''),
  ...
  function loadTakimFile(file) {
    ...
    readExcelFile(file, async (err, wb) => {
      ...
      const cols = {
        ...
        takimKod: findColumn(keys, ['TAKIM', 'KOD']) || findColumn(keys, ['TAKIMKOD']),
        anaGrup: findColumn(keys, ['ANA', 'GRUP']) || findColumn(keys, ['ANAGRUP']),
        altGrupAciklama: findColumn(keys, ['ALT', 'GRUP', 'AÇIKLAMA']) || findColumn(keys, ['ALT', 'GRUP']) || findColumn(keys, ['ALTGRUP']),
      };
      ...
      for (const r of raw) {
        ...
        if (!kod) continue;
        state.takimMap[kod] = {
          malGrubu: String(r[cols.malGrubu] || '').trim(),
          sezonDurumu: String(r[cols.sezonDurumu] || '').trim(),
          takimDurumu: String(r[cols.takimDurumu] || '').trim(),
          takimKod: String(r[cols.takimKod] || '').trim(),
          anaGrup: String(r[cols.anaGrup] || '').trim(),
          altGrupAciklama: String(r[cols.altGrupAciklama] || '').trim(),
        };
      }
  ...
  function loadIrsaliye(event) {
    const file = event.target.files[0];
    loadIrsaliyeFile(file);
  }

  function loadIrsaliyeFile(file) {
    if (!file) return;
  ...
  function detectFileType(file) {
    const name = String(file && file.name || '').toLocaleLowerCase('tr-TR');
    if (!/\.(xlsx|xls)$/i.test(name)) return null;
    if (name.includes('takım') || name.includes('takim') || name.includes('mal') || name.includes('grup')) return 'takim';
    if (name.includes('irsaliye') || name.includes('performans')) return 'irsaliye';
    return 'nebim';
  }

  function loadDroppedFiles(fileList, preferredType) {
    const files = Array.from(fileList || []).filter(f => /\.(xlsx|xls)$/i.test(f.name));
    if (!files.length) { alert('Lütfen .xlsx veya .xls dosyası sürükleyin.'); return; }
    for (const file of files) {
      const type = preferredType || detectFileType(file);
      if (type === 'takim') loadTakimFile(file);
      else if (type === 'irsaliye') loadIrsaliyeFile(file);
      else loadNebimFile(file);
    }
  }
  ...
  function exportExcel() {
    ...
    const d1 = (UI.getFilteredDepo ? UI.getFilteredDepo() : a.depoTransfers).map(t => ({
      ...
      'Gönderen Depo': t.gonderenDepoAdi || 'Merkez Depo',
      'Takım Bilgisi': t.takimDurumu,
      'Takım Kodu': t.takimKod || '',
    ...
    setColWidths(ws1, [10, 12, 16, 22, 22, 14, 8, 10, 10, 36, 12, 8, 30]);
    ...
    setColWidths(ws2, [10, 12, 12, 16, 22, 22, 14, 8, 6, 12, 8, 36]);
    ...
    if (a.kirikBeden.length) {
      const d3 = a.kirikBeden.map(t => ({
        ...
        'Beden': t.beden,
        'Adet': t.adet,
        'Üretilen Beden Sayısı': t.uretilenBedenSayisi || '',
        'Mevcut Beden Sayısı': t.mevcutBedenSayisi || '',
        'Lokasyon Key': t.lokasyonKey || '',
        'Hedef': t.hedef.label,
        'Neden': t.neden,
      }));
